(function(){
  ...
  Renderer = function(canvas){
      ...
      redraw:function(){
        ...
        particleSystem.eachNode(function(node, pt){
          ...
          /***********/
            //validamos el tipo de nodo para ponerle color
 			...
 			ctx.fillStyle="rgb(255,255,81)"
                ...
                else if(node.data.tipo=="orange"){
 			ctx.fillStyle="rgb(255,102,0)"
 			}

                
                else if(node.data.tipo=="red"){
 			ctx.fillStyle="rgb(204,0,0)"
 			}
                
                else if(node.data.tipo=="green"){
          ...
          if (label){
            ...
            ctx.fillText(label||"", pt.x, pt.y+w/2+10)
            ctx.fillText(label||"", pt.x, pt.y+w/2+10)
          }
  })    			
    ...
    var desc = edge.data.desc||""      
   ctx.fillText ((desc), (pt1.x + pt2.x) / 2,   
   (pt1.y + pt2.y) / 2);

     var neigh_ip = edge.data.neigh_ip||""      
  ctx.fillText ((neigh_ip), (pt1.x + pt2.x) / 2,   
  ((pt1.y + pt2.y) / 2)+15);

   //  { 
//var info = new Image();
//info.src = 'qmark.png' ;
//ctx.drawImage(info, (pt1.x + pt2.x) / 2,  (pt1.y + pt2.y) / 2);
//}       
...
var weight="1"
      ...
      initMouseHandling:function(){
        ...
        var handler = {
          ...
          //alert("Hi! You selected "+ selected.node.data.label);
          //window.location.href = 'http://localhost/myrouter/router.php?file='+selected.node.data.label;
